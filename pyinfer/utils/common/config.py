...
class ConfigDict(Dict):
    def __init__(self, config_dict):
        """支持三种环境变量"""
        self.pattern_dict = {re.compile('{{ AppFolder }}'): os.environ["AppFolder"],
                             re.compile('{{ WorkspaceFolder }}'):os.environ["WorkspaceFolder"],
                             re.compile('{{ RootFolder }}'):os.environ["RootFolder"]}

        config_dict = self.identify_env_variables(config_dict) 
        self.__delattr__("pattern_dict")
        super().__init__(config_dict)
    
    def identify_env_variables(self, config_dict):
        for k, v in config_dict.items():
            if isinstance(v,dict):
                config_dict[k] = self.identify_env_variables(v)
            elif isinstance(v,str):
                for pattern , repl in super().__getattr__('pattern_dict').items():
                    new_v = re.sub(pattern, repl, v)
                    if new_v != config_dict[k]:
                        config_dict[k] = new_v
                        break
        return config_dict
